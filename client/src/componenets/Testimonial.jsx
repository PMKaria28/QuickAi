                ...
                {dummyTestimonialData.map((testimonial, index) => (
                    <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition duration-300 cursor-pointer'>
                        {/* <div className="flex items-center gap-1">
                            {Array(5).fill(0).map((_,img)=>(<img key={index} src={index < testimonial.image ? assets.star_icon : assets.star_dull_icon}  className="w-4 h-4" alt="star"/>))}
                        </div> */}
                         <div className="flex items-center gap-1">
                            {Array(5).fill(0).map((_, imgIndex) => (
                                <img
                                    key={imgIndex}
                                    src={imgIndex < testimonial.rating ? assets.star_icon : assets.star_dull_icon}
                                    className="w-4 h-4"
                                    alt="star"
                                />
                            ))}
